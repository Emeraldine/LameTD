...
canvas.addEventListener('mousemove', e => {
    if(selected) {
        ...
        {
            if(e.x >= Number(placedTowers[i].x) - 26 && e.x <= Number(placedTowers[i].x) + 26 && e.y >= Number(placedTowers[i].y) - 30 && e.y <= Number(placedTowers[i].y) + 32)
            {
                canPlace = false;
            } else {
                canPlace = true;
...
canvas.addEventListener('mousedown', e => {
    if(selected) {
        if(canPlace) {
            let tmp = towerMap.find(tower => tower.name == selection);
            drawTower(tmp.image,e.x,e.y);
...
function rebuild() {
    setBg(source);
    for(let i = 0; i < placedTowers.length; i++) {
        let t = towerMap.find(tower => tower.name == placedTowers[i].type);
        let tmp = new Image();
        tmp.src = t.image;
        tmp.onload = function() {
